---
title: "Semi-synthetic data simulation"
...
results_per_lambda <- readRDS("./Results/UniformSynthetic.rds")
datasets_per_lambda <- 100
factors <- seq(0, 0.5, 0.1)
```
...
#saveRDS(results_per_lambda, "./Results/UniformSynthetic.rds")
...
for (i in seq_along(datasets)) {
  for (j in seq_along(datasets[[i]])) {
    ...
    lambda_val <- attributes(datasets[[i]][[j]])$incre
...
unique(df_long$CP_Label)

p5 <- ggplot(df_long, aes(x = CP_Label, y = Value, color = PairType)) +
