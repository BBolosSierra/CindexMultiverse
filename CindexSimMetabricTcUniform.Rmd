...
for (i in seq_along(factors)) {
  for (j in seq_len(10)) {
    seed = sample(1:1e6, 1)
    cat("Seed: ", seed, "\n ")
    # Run the function with different factor.cens_c_factor values
    ...
    datasets_weibull[[i]][[j]] <- add_censoring(my_surv_data = dat,
                          ...
                          cens.params = list(
                            weibull_cens_model = survreg_model_c, 
                            factor.cens_c_factor = factors[[i]]
                          ),
                          seed = 123)
    ...
    attr(datasets_weibull[[i]][[j]], "factor.cens") <- factors[[i]]
    attr(datasets_weibull[[i]][[j]], "cp") <- cp
    
    cat("factor.cens: ", factors[[i]], " % Censoring: ", cp, "\n")
    ...
    attr(datasets_inform[[i]][[j]], "factor.cens") <- factors[[i]]
