...
```{r Histogram of uncensored event times}
...
To study the effect of ties, the number of discretizations of the event times is increased. 

We will use the results from the 5foldCV, and generate bootstrap samples to estimate the C-index confidence intervals. We also estimate the C-index point estimates. 
...
```{r Number unique of event times after Rounding(T,1) }
...
Based on the resample indices for the bootstrap, we generate subsets of the data, that contain the right input type (expected mortality, survival probabilities or RMST), for the right models. 

We will re-use the same bootstrap samples for all the discretizations. 
...
```{r Generate subsets from bootstrap indexes. Rounding(T, 1)}
subset_expm_TT <- list()
subset_surv_TT <- list()
subset_rmst_TT <- list()
for (r in seq_along(resample_indices)) {
...
Estimation of the C-index based on just RMST and survival. The implementations are chosen for their ability to turn on and off the inclusion of ties in times. Therefore only pec is tested here for $C_\tau$, and pycox is tested for $C_{td}$. However, any of the implementations we have gathered in this study estimating $C$ allow for inclusion/exclusion of ties based on user-defined parameters. 
...
```{r Estimation of C-index Rounding(T,1)}
# Get the right columns
select_rmst_TT <- grep("RMST\\.",colnames(subset_rmst_TT[[1]]), value = TRUE)
# Generate the empty list
results_rmst_pe_incl_ties_TT <- list() 
results_rmst_pe_excl_ties_TT <- list() 
results_rmst_bo_excl_ties_TT <- list()
results_rmst_bo_incl_ties_TT <- list()
...
for (rmst in select_rmst_TT) {
   ...
   # Run the inclusion of ties
   # Bootstrap
   results_rmst_bo_incl_ties_TT[[rmst]] <- bootstrap.metric.parallel(metrics.wrapper2,
                                  dataset=list(
                                     predicted = rmst,
                                     censoring = "test_status",
                                     time = "test_time"),
   ...
   # Point estimate
   results_rmst_pe_incl_ties_TT[[rmst]] <- metrics.wrapper2(predicted = stacked_predictions_TT[[rmst]],
   ...
   # Run the exclusion of ties
   # Bootstrap
   results_rmst_bo_excl_ties_TT[[rmst]] <- bootstrap.metric.parallel(metrics.wrapper2,
                                  dataset=list(
                                     predicted = rmst,
                                     censoring = "test_status",
                                     time = "test_time"),
   ...
   # Point estimate
   results_rmst_pe_excl_ties_TT[[rmst]] <- metrics.wrapper2(predicted = stacked_predictions_TT[[rmst]],
                                                        censoring = stacked_predictions_TT$test_status,
                                                        time = stacked_predictions_TT$test_time,
                                                        implementation = list("pec::cindex"),
                                                        eval.times = 120,
...
results_surv_bo_TT <- list()
results_surv_pe_TT <- list()
for (model in model_names) {
  cat("Calculating bootstrap for Distribution for Model = ", model, "\n")
  # Bootstrap
  results_surv_bo_TT[[model]] <-  bootstrap.metric(metrics.wrapper,
                                  dataset=list(
                                     predicted = model,
                                     censoring = "test_status",
                                     time = "test_time"),
  ...
  # Generate matrix to estimate 
  surv_mat = stacked_predictions_TT[, grep(paste0("^", model), names(stacked_predictions_TT), 
                                        value = TRUE), drop = FALSE]
  colnames(surv_mat) <- as.numeric(sub(".*\\.", "", colnames(surv_mat)))
  # Point estimate
  results_surv_pe_TT[[model]] <- metrics.wrapper(surv_matrix = surv_mat, 
                                                    censoring = stacked_predictions_TT$test_status, 
                                                    time = stacked_predictions_TT$test_time, 
                                                    implementation = list("pycox.Ant",
                                                                          "pycox.Adj.Ant"))
...
rmst_pec_excl_ties_TT$Notation   <- "C_tau_10"
rmst_pec_incl_ties_TT$Notation   <- "C_tau_10"
...
rmst_pec_incl_ties_TT$Incl.Ties.Times  <- rmst_pec_incl_ties_TT$Metric
...
rmst_pec_excl_ties_TT$Incl.Ties.Times  <- paste0(rmst_pec_excl_ties_TT$Metric, "*")
...
rmst_pec_excl_ties_TT$Rounding   <- "Round(T,1)"
rmst_pec_incl_ties_TT$Rounding  <- "Round(T,1)"
...
df_plot <- bind_rows(surv_df_plot, rmst_pec_excl_ties, rmst_pec_incl_ties, 
                     surv_df_plot_TT, rmst_pec_excl_ties_TT, rmst_pec_incl_ties_TT,
...
rmst_pec_incl_ties_TT$Incl.Ties.Times  <- TRUE
rmst_pec_incl_ties_T2$Incl.Ties.Times  <- TRUE

rmst_pec_excl_ties$Incl.Ties.Times     <- FALSE
rmst_pec_excl_ties_TT$Incl.Ties.Times  <- FALSE
...
We would create three different models based on increasing discretization of the age as covariates. After we would estimate the C-index with those implementations where we can change the inclusion of ties in predictions based on user-defined parameters. Those are pysurvival and Hmisc for $C$, pec for $C_\tau$ and pycox for $C_\{td}$
...
```{r Format and plot fig.width=7, fig.height=8}
# Add notation
surv_df_plot$Notation           <- "C_td"
rmst_df_plot_incl_ties$Notation <- ifelse(rmst_df_plot_incl_ties$Metric == "pec::cindex",
                                          "C_tau_10", "C")
rmst_df_plot_excl_ties$Notation <- ifelse(rmst_df_plot_excl_ties$Metric == "pec::cindex", 
                                          "C_tau_10", "C")
...
rmst_df_plot_incl_ties$Incl.Ties.Preds  <- rmst_df_plot_incl_ties$Metric
...
```{r Format and plot 2  fig.width=8, fig.height=6}

# Add notation
surv_df_plot$Notation           <- "C_td"
rmst_df_plot_incl_ties$Notation <- ifelse(rmst_df_plot_incl_ties$Metric == "pec::cindex", "C_tau_10", "C")
rmst_df_plot_excl_ties$Notation <- ifelse(rmst_df_plot_excl_ties$Metric == "pec::cindex", "C_tau_10", "C")
