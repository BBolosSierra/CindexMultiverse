...
cat("Number of unique times: ", length(unique(stacked_predictions$test_time)), "\n")
...
cat("New number of pairs that have tied times: ", num_tied_pairs, "\n")
cat("New % of tied pairs out of total possible pairs ", num_tied_pairs/n_total_pairs *100, "\n")
...
which will be shared across all models. 
...
The same is done for the dataset with rounded times (note that the predictions
are the same, but we generate the full object to keep the same format).
...
## `pec` with and without ties

Here, we apply `pec` to the stacked predictions with and without considering
ties in times. We do this using RMST as input. 

```{r pec with and wihout ties in times}
select_rmst <- grep("RMST\\.",colnames(subset_rmst[[1]]), value = TRUE)

# Point estimate + bootstrap results including ties
results_rmst1_pe_ties <- list() 
results_rmst1_b_ties <- list()

# Point estimate + bootstrap results excluding ties
