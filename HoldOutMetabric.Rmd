...
for (fold_n in unique(folds_stacked_predictions$cv_fold)) {
  ...
  cat("Maximum time in fold ", fold_n, " is ", max(stacked_predictions$test_time) ," and maximum uncensored time is ",max_uncensored_time, "\n")
}
```
...
```{r Plots, fig.width=13, fig.height=6}
# Extract the plots
for (fold_n in unique(folds_stacked_predictions$cv_fold)) {
  ...
  ## Test
  tmp <- folds_stacked_predictions[folds_stacked_predictions$cv_fold == fold_n, ] 
  # revert the status, so now censor is 1 
  tmp$censor.status <- ifelse(tmp$test_time %in% c(1, 2), 1, 0)
  # Order
  tmp <- tmp[order(tmp$test_time),]
  # Calculate weight by time points
  weight.j <- pec::ipcw(formula=Surv(test_time,censor.status)~1,
                   data=tmp,
                   method="marginal",
                   times=unique(tmp$test_time),
                   ...
                   subjectTimesLag=0,
                   what="IPCW.times")$IPCW.times
  # Calcualte the inverse of weights: IPCW
  IPCWtest <- 1 / weight.j^2
  # Extract time points
  time_points_test <- unique(tmp$test_time)
  ...
  plot(time_points_test, IPCWtest,
       ...
       xlab = "Time (months)", 
       ylab = "IPCW")
  lines(time_points, IPCWtrain ,col="darkorange")
  legend("topright",                         
       legend = c("Test IPCW", "Train IPCW"), 
