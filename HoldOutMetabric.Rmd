...
```{r load results}
folds_stacked_predictions <-  readRDS("./Results/5FoldStackedPredictions2.rds")

survival_curves <- readRDS("./Results/5FoldStackedPredictionsCurves2.rds")
```
...
```{r Fold 1 Censoring and IPCW, fig.width=10}
...
```
```{r Expected mortality vs RiskScore}
ggplot(folds_stacked_predictions, aes(x= RMST.CoxTime, y = ExpMort.CoxTime)) +
  geom_point(alpha = 0.6) +
  facet_wrap(~ cv_fold) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(
    title = "Relationship between RMST.CoxTime and ExpMort.CoxTime",
    x = "AUSC (-sum (S(t) * dt))",
    y = "Expected Mortality "
  ) +
  theme_minimal()

ggplot(folds_stacked_predictions, aes(x = RMST.DeepSurv, y = ExpMort.DeepSurv)) +
  geom_point(alpha = 0.6) +
  facet_wrap(~ cv_fold) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(
    title = "Relationship between RMST..DeepSurv and ExpMort.DeepSurv",
    x = "AUSC (-sum (S(t) * dt))",
    y = "Expected Mortality  "
  ) +
  theme_minimal()

ggplot(folds_stacked_predictions, aes(x = RMST.DeepHit, y = ExpMort.DeepHit)) +
  geom_point(alpha = 0.6) +
  facet_wrap(~ cv_fold) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(
    title = "Relationship between RMST.DeepHit and ExpMort.DeepHit",
    x = "AUSC (-sum (S(t) * dt))",
    y = "Expected Mortality "
  ) +
  theme_minimal()

ggplot(folds_stacked_predictions, aes(x = RMST.CoxPH, y = ExpMort.CoxPH)) +
  geom_point(alpha = 0.6) +
  facet_wrap(~ cv_fold) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(
    title = "Relationship between RMST.CoxPH and ExpMort.CoxPH",
    x = "AUSC (-sum (S(t) * dt))",
    y = "Expected Mortality "
  ) +
  theme_minimal()

ggplot(folds_stacked_predictions, aes(x = RMST.RSF, y = ExpMort.RSF)) +
  geom_point(alpha = 0.6) +
  facet_wrap(~ cv_fold) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(
    title = "Relationship between RMST.RSF and ExpMort.RSF",
    x = "AUSC (-sum (S(t) * dt))",
    y = "Expected Mortality  "
  ) +
  theme_minimal()
...
for (fold_n in unique(folds_stacked_predictions$cv_fold)) {
  ...
  df_plot <- bind_rows(risk_df_plot1, risk_df_plot2, risk_df_plot3, surv_df_plot)
