...
### Simulation data: age-informative Weibull.  
...
datasets <- load_synthetic_datasets("./Datasets/Synthetic/10d_synthetic_WT_ITc.rds")
...
#saveRDS(stacked_predictions, "./Results/InformativeSyntheticStackedPredictions.rds")

stacked_predictions <- readRDS( "./Results/InformativeSyntheticStackedPredictions.rds")
...
```{r}
results_per_lambda <- readRDS("./Results/InformativeSynthetic.rds")
datasets_per_lambda <- 100
factors <- c(0, 0.5, 1.5, 3, 7, 13)
```
...
#saveRDS(results_per_lambda, "./Results/InformativeSynthetic.rds")
...
p3 <- ggplot(df_long, aes(x = CP_Label, y = Value, fill = PairType)) +
  geom_boxplot(alpha = 0.5, outlier.shape = NA) +
  facet_wrap(~ Group + Measure, labeller = label_parsed) +
  scale_fill_manual(
    values = c("Comparable" = "#FB9A99", "Concordant" = "white"),
    labels = c("Comparable" = "Weighted Sum Comparable",
               "Concordant" = "Weighted Sum Concordant")
  ) +
  labs(x = "", y = "Number of pairs", fill = NULL) +
  theme_minimal(base_size = 12) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),  legend.position = "bottom")

p3 <- ggplot(df_long, aes(x = CP_Label, y = Value, color = PairType)) +
