...
datasets <- load_synthetic_datasets("./Datasets/Synthetic/10d_synthetic_WT_ITc_v2.rds")
...
```{r}
df_hist <- data.frame(
  value = c(test$time, test$censoring_time, test$observed_time),
...
```{r}
#saveRDS(results_per_lambda, "./Results/UniformSynthetic_v2.rds")

```

```{r}
results_per_lambda <- readRDS("./Results/UniformSynthetic_v2.rds")
```
...
final_df$cp_value <- cp_list[match(final_df$Lambda, lambdas)]
```
...
df_long$cp_value <- cp_list[match(df_long$lambda, lambdas)]
# First plot
...
```
```{r}
ggplot(final_df[final_df$Lambda == 1.5,],  
