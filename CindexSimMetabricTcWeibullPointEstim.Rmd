...
structured_datasets_inform <- list(
  ...
  attributes = lapply(datasets_uniform, function(factor_list) {
    lapply(factor_list, function(dataset) {
      attributes(dataset)  # Extract attributes of each dataset
    })
  })
...
#results_expm1 <- vector("list", length = datasets_per_lambda)
#results_expm2 <- vector("list", length = datasets_per_lambda)
results_expm3 <- vector("list", length = datasets_per_lambda)
...
for (l in seq_along(stacked_predictions)) { 
  ...
  for (dataset in seq_along(subset)) {
    ...
    for (r in seq_along(resample_indices)){
     ...
     for (exps in selected_exps){
        cat("Boostrapping for lambda ", lambda, ", dataset ", 
  ...
  #results_per_lambda[[l]] <- list(results_expm1, results_expm2,
  #                                results_expm3, results_surv)
  results_per_lambda[[l]] <- list(results_expm3, results_surv)
}
...
#  list_exp1 <- results_per_lambda[[l]][[1]]
#  list_exp2 <- results_per_lambda[[l]][[2]]
  list_exp  <- results_per_lambda[[l]][[1]]
  list_surv <- results_per_lambda[[l]][[2]]
  
  lambda <- factors[[l]]

  # Extract raw entries
#  raw_exp1 <- extract_expm_entries(list_exp1, lambda)
#  raw_exp2 <- extract_expm_entries(list_exp2, lambda)
  raw_exp <- extract_expm_entries(list_exp, lambda)
  raw_surv <- extract_surv_entries(list_surv, lambda)
...
```{r}
ggplot(final_df[(final_df$Notation == "C_tau" | final_df$Notation == "C"),],  aes(x = as.factor(Lambda), y = Cindex, fill = Metric)) +
...
for (l in seq_along(results_per_lambda)) {
  ...
  list_exp3 <- results_per_lambda[[l]][[1]]

  lambda <- factors[[l]]

  raw_exp3 <- extract_expm_pairs(list_exp3, lambda,  extract = extract)
...
df_pairs$survival.n.G2.CompPairs <- df_pairs$survival.n.G2.CP + df_pairs$survival.n.G2.DP
df_pairs$survival.n.CompPairs <- df_pairs$survival.n.CP + df_pairs$survival.n.DP

df_pairs$survival.n.G2.ConPairs <- df_pairs$survival.n.G2.CP 
df_pairs$survival.n.ConPairs <- df_pairs$survival.n.CP
...
# lambda, dataset, survival.n.G2.CompPairs, survival.n.G2.CP, survival.n.CompPairs, survival.n.CP

# Reshape to long format
df_long <- df_pairs[,
  c("survival.n.G2.ConPairs", "survival.n.G2.CompPairs", 
         "survival.n.ConPairs", "survival.n.CompPairs", "lambda", "dataset")] %>%
  pivot_longer(
...
# First plot
p1 <- ggplot(df_long, aes(x = as.factor(lambda), y = Value, fill = Metric)) +
  geom_boxplot(alpha = 0.5, outlier.shape = NA) +
  facet_wrap(~ Group, scales = "fixed") +
  labs(x = "Lambda", y = "Number of pairs", title = "") +
...
p2 <- ggplot(final_df[((final_df$Notation == "C_tau" | final_df$Notation == "C") & 
             ...
             aes(x = as.factor(Lambda), y = Cindex, fill = Metric)) +
  geom_violin(alpha = 0.4, color = "black") +
  geom_errorbar(aes(ymin = Lower, ymax = Upper), position = position_dodge(width = 0.9), width = 0.25) +
  facet_wrap(~ Metric) +
  labs(title = "",
