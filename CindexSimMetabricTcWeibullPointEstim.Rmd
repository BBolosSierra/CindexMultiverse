...
Do not run this one if the previous code was run. 

```{r Load results checkpoint, eval=FALSE}
...
dat_plot <- final_df[(final_df$Metric %in% c("Hmisc::rcorr.cens", "pec::cindex", "pycox.Ant")),]

dat_plot <- dat_plot %>% 
  filter(!(Implementation == "Hmisc" & 
             Measure == "tilde(C)[tau]~(RMST~tau==100~months)"))

oracle_means <- dat_plot |>
  filter(Model == "Oracle") |>
  group_by(Implementation, Measure) |>
  summarise(
    oracle_mean = median(Cindex, na.rm = TRUE),
...
dat_plot <- final_df %>%
  filter(Metric %in% c("Hmisc::rcorr.cens", "pec::cindex", "pycox.Ant")) %>%
  filter(!(Implementation == "Hmisc" & 
           Measure == "tilde(C)[tau]~(RMST~tau==100~months)"))
...
# Join back to get Oracle values for difference calculation
dat_plot <- dat_plot %>%
  left_join(oracle_means, by = c("Implementation", "Measure")) %>%
  mutate(Cindex_diff = Cindex - oracle_mean)

# Plot difference from Oracle for CoxPH only
ggplot(dat_plot[dat_plot$Model == "CoxPH", ], 
       aes(x = CP_Label, y = Cindex_diff, fill = Notation)) +
  geom_violin(alpha = 0.2, color = "black") +
  stat_summary(
    fun = median, geom = "point", shape = 21,
    size = 2, fill = "black", color = "black",
  ...
  # Oracle SD band (now around 0)
  geom_rect(
    data = oracle_means,
    aes(xmin = -Inf, xmax = Inf,
        ymin = -oracle_sd, ymax = oracle_sd),
    fill = "grey40", alpha = 0.1, inherit.aes = FALSE
  ) +
  # Oracle horizontal line (now at 0)
  geom_hline(
    yintercept = 0,
    color = "black", linetype = "dashed", linewidth = 0.7
  ) +
  facet_wrap(~Implementation + Measure, labeller = label_parsed) +
  scale_fill_discrete(labels = scales::parse_format()) +
  labs(y = expression((C~index) - (C~index)^oracle), x = "Censoring percentage") +
  theme_minimal(base_size = 12) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "bottom"
...
```{r fig.width=8, fig.height=9}
# Step 1: Filter for relevant models and metrics
df <- final_df %>%
  filter(Model %in% c("CoxPH", "Oracle"),
         Implementation %in% c("Hmisc", "pec", "pycox.Ant")) %>%
  filter(!(Implementation == "Hmisc" & 
           Measure == "tilde(C)[tau]~(RMST~tau==100~months)"))

# Step 2: Make a unique identifier for each comparison group
df$GroupID <- paste(df$Implementation, df$Measure, df$Dataset, df$Lambda, sep = "_")

# Step 3: Reshape to wide format (CoxPH vs Oracle)
df_wide <- reshape(
  df[, c("GroupID", "Model", "Cindex")],
  timevar = "Model",
  idvar = "GroupID",
  direction = "wide"
)

# Step 4: Parse out useful columns from GroupID
...
df_wide$Lambda <- group_split[, 4]

# Step 5: Compute difference: CoxPH - Oracle
df_wide$CindexDiff <- df_wide$Cindex.CoxPH - df_wide$Cindex.Oracle

# Step 6: Relabel Lambda -> Censoring percentage
df_wide$DeltaCP_Label <- dplyr::case_when(
  df_wide$Lambda == "0"   ~ "0%",
  df_wide$Lambda == "0.5" ~ "14.2%–21.1%",
  df_wide$Lambda == "1.5" ~ "30.8%–39.7%",
  df_wide$Lambda == "3"   ~ "43.3%–51.2%",
  df_wide$Lambda == "7"   ~ "58.4%–65.8%",
  df_wide$Lambda == "13"  ~ "68.6%–75.7%",
  TRUE ~ NA_character_
)

# Step 7: Add Notation labels
df_wide$Notation <- dplyr::case_when(
  df_wide$Implementation == "pec" ~ "tilde(C)[tau]~(RMST~tau)",
  df_wide$Implementation == "pycox.Ant" ~ "tilde(C)[td]~(Survival~Distribution)",
  df_wide$Implementation == "Hmisc" ~ "tilde(C)~(RMST)",
  TRUE ~ NA_character_
)
...
# Step 9: Plot
ggplot(df_wide, aes(x = as.factor(DeltaCP_Label), y = CindexDiff, fill = Notation)) +
  geom_violin(alpha = 0.2, color = "black") +
  stat_summary(fun = median, geom = "point", shape = 21, 
               size = 2, fill = "black", color = "black", 
               position = position_dodge(width = 0.9)) +
  ...
  labs(
    x = "Censoring percentage",
    y = expression(C*"-index"^"CoxPH" - C*"-index"^"Oracle")
  ) +
  theme_minimal(base_size = 12) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
...
```{r Ploting comparable concordant, fig.width=7, fig.height=6}
...
dat <- final_df[(final_df$Metric %in% c("survival.n", "survival.n/G2") & 
                   (final_df$Measure %in% "tilde(C)[tau]~(RMST~tau==100~months)")),]
...
df_long$Measure <- "tilde(C)[tau]~(RMST~tau==100~months)"
