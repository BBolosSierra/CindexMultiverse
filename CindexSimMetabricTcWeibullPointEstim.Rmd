...
```{r}
#saveRDS(stacked_predictions, "./Results/WeibullSyntheticStackedPredictions_v2.rds")
```

```{r}
stacked_predictions <- readRDS( "./Results/WeibullSyntheticStackedPredictions.rds")
#stacked_predictions <- readRDS( "./Results/WeibullSyntheticStackedPredictions_v2.rds")
```
...
final_df$cp_value <- cp_list[match(final_df$Lambda, factors)]
```
...
#### Need to test antolinis in R too: 


Select a dataset with high degree of censoring: 
...
subset <- stacked_predictions[[5]]

results_surv_Km <- vector("list", length = 10)
...
for (dataset in seq_along(subset)) {
  ...
  for (model in model_names) {
    # Antolinis
    cat("Calculating bootstrap for Distribution for Model = ", model, "\n")
    results_surv_Km[[dataset]][[model]] <-  bootstrap.metric(metrics.wrapper,
                                    dataset=list(
                                       predicted = model,
                                       censoring = "test_status",
                                       time = "test_time"),
...
pycoxR_SurvMatrix_Ant <- function(time, surv_matrix, censoring){
  ...
  ev <- EvalSurv(surv_df, time_py, censoring_py)
...
results_surv_noKm <- vector("list", length = 10)
...
for (dataset in seq_along(subset)) {
  ...
  for (model in model_names) {
    # Antolinis
    cat("Calculating bootstrap for Distribution for Model = ", model, "\n")
    results_surv_noKm[[dataset]][[model]] <-  bootstrap.metric(metrics.wrapper,
                                    dataset=list(
                                       predicted = model,
                                       censoring = "test_status",
                                       time = "test_time"),
